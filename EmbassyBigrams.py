...
arg_parser.add_argument("--pages", help="Specify whether to use all facebook pages, only US facebook pages, or only Guyana faebook pages. Default is us", choices=['all','us','guy','embassy'], default='us')
...
for page in page_comments: 
    ...
    #keep top n bigrams
    bigram_count = collections.Counter(bigrams_list).most_common(args.num_bigrams)
...
for bigram in all_bigrams:
    if ('visa' in bigram)|('visas' in bigram):
        visas.append(bigram)
    if bigram[0] != bigram[1]:
        overall_bigrams.append(bigram)
visas = collections.Counter(visas)
print(visas)
all_bigram_counts = collections.Counter(overall_bigrams).most_common(args.num_bigrams)
